...
export const UserDetail = () => {
  ...
  const user: User[] = useSelector(userSelector).filter(
    (item: User) => item.id == idUser
  );
  ...
  const fetchOrdersByDates = async (
    ...
    const endURL =
      startDate && endDate
        ? `${idUser}&${dateToString(startDate)}&${dateToString(endDate)}`
        : `${idUser}&${stDate}&${edDate}`;
                ...
                orders.map((order: Order, i: number) => (
                  <tr key={i}>
                    <td>
                      ...
                      {+order.creationDate.split(" ")[1].substring(0, 2) - 3 >=
                      12
                        ? "PM"
                        : "AM"}
                    </td>
                    ...
                    <td>
                      <NavLink
                        to={`/statistics/clients/${idUser}/order/${order.id}`}
                      >
